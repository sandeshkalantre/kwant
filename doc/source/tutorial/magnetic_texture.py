...
def plot_vector_field(syst, params):
    ...
    y = list(range(ymin, ymax + 1))

    m_i = [field_direction(p, **params) for p in itertools.product(x, y)]
    m_i = np.reshape(m_i, (len(x), len(y), 3))
...
def plot_currents(syst, currents):
    ...
    for ax, (title, current) in zip(axes, currents):
        kwant.plotter.plot_current_density(
...
#HIDDEN_BEGIN_ldos
    ...
    probability = np.abs(up)**2 + np.abs(down)**2
#HIDDEN_END_ldos
...
#HIDDEN_BEGIN_bind
    ...
    J_m_left = sum(J_m_bound(p) for p in psi_from_left)
    J_z_left = sum(J_z_bound(p) for p in psi_from_left)
#HIDDEN_END_bind
