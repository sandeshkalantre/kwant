...
:math:`a`.  For example, to calculate the local current and the z-projected
spin current:
...
The evaluating a `~kwant.operator.Current` operator on a wavefunction returns a
1D array of values that can be directly used with `kwant.plotter.vector_map`:

.. image:: ../images/spin_currents.*

.. note::

    The evaluating a `~kwant.operator.Current` operator on a wavefunction
...
.. specialnote:: Technical Details
    ...
    where is is equal to the hopping matrix :math:`\mathbf{H}_{ab}` between
    these two sites.
